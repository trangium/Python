...
#    global abcache
#    abcache = {}
    return alphabeta(board, mover, -65, 65, 0)[:]

def alphabeta(board, mover, lower, upper, depth):
#    if (board, mover) in abcache: return abcache[(board, mover)][:]
    if not getPossibleMoves(board, mover):
        if not getPossibleMoves(board, TOKENINVERSE[mover]):
            return [board.count(mover) - board.count(TOKENINVERSE[mover])]
        ab = alphabeta(board, TOKENINVERSE[mover], -upper, -lower, depth+1)[:] + [-1]
        ab[0] *= -1
        return ab
    ...
    if depth <= 3:
        possibleMoves.sort(key=lambda pos: alphaMoveScore(board, mover, pos), reverse=True)
    for pos in possibleMoves:
        ab = alphabeta(boardOnMove(board, pos, mover), TOKENINVERSE[mover], -upper, -lower, depth+1)[:]
        score = -ab[0]
        if score < lower: continue
        if score > upper: return [score]
        best = ab + [pos]
        best[0] *= -1
        if depth == 0: print(best)
        lower = score + 1
#    abcache[(board, mover)] = best[:]
    return best

def alphaMoveScore(board, mover, pos):
...
def showBoard(board, possibleMoves, mover, recentMove=-1):
    ...
    if not possibleMoves: pass # print("No moves possible")
...
def main():
    ...
    moveStack = []
    for arg in args[::-1]:
        if arg[:2] == "HL": MAXHOLES = int(arg[2:])
        elif arg in "XxOo": mover = arg
