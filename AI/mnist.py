...
def randweights():
   layer1 = [random.uniform(-1/28, 1/28) for i in range(784*16)] 
   layer2 = [random.uniform(-1/4, 1/4) for i in range(16*160)] 
   layer3 = [random.uniform(-1/12, 1/12) for i in range(160*10)]
...
def main():
   ...
   for i in range(10):
      for target, mapixels in data:
         ...
         classifier = smart_bp(classifier, pixels, target=[int(a == target) for a in range(10)], alpha=0.003)
         ct += 1
         if ct % 1000 == 0:
            print(ct)
