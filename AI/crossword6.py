...
for i in range(6*boardSize):
    ...
    for elem in pareto:
        if elem[1] <= nextGoodness and elem[2] == nextBlocks: # elem is strictly worse than the new board
            pareto.remove(elem)
        if elem[1] >= nextGoodness + boardSize/(40*random.random()) and elem[2] == nextBlocks: # elem is strictly better than the new board
...
board = getFinal(board, brd)
printBoard(board)
print(score, blocks, "\n\n")

# Now to solve the crossword.

def posRow(pos): return pos // boardWidth

def posCol(pos): return pos % boardWidth

def getLines(board):
    transposedIndices = [j for i in range(boardWidth) for j in range(i, boardSize, boardWidth)]
    lines = []
    currentLn = []
    for pos in range(boardSize):
        if board[pos] != BLOCKCHAR: currentLn.append(pos)
        if posCol(pos) == boardWidth-1 or board[pos+1] == BLOCKCHAR:
            if currentLn: lines.append(currentLn)
            currentLn = []
    for pos in transposedIndices:
        if board[pos] != BLOCKCHAR: currentLn.append(pos)
        if posRow(pos) == boardHeight-1 or board[pos+boardWidth] == BLOCKCHAR:
            if currentLn: lines.append(currentLn)
            currentLn = []
    return lines

def fillLine(board, line, word):
    if len(line) != len(word): return ""
    for idx, pos in enumerate(line):
        if board[pos] == BLOCKCHAR: return ""
        if board[pos] != OPENCHAR and board[pos] != word[idx]: return ""
    return "".join(val if idx not in line else word[line.index(idx)] for idx, val in enumerate(board))
