...
def goodness(row):
    ...
    good = 0
    for n in counts:
        if n <= 7: good += [0, -1, -1, 5, 4, 3, 2, 1][n]
        else: good -= (n-8)**2
    return good

def fullGoodness(board):
    good = -board.count(BLOCKCHAR)*1.5 + 0.2/random.random()
...
def weightedRand(board):
    ...
    for row in range(boardHeight):
        ...
        for col in range(boardWidth):
            ...
            weightArray[row*boardWidth+col] = max(0.01, (nextGood-currentGood))**0.3 if board[row*boardWidth+col] != BLOCKCHAR else 0
...
while True:
    ...
    for i in range(30):
        ...
        prevBoard = blockBoard[:]
        fillOnce(blockBoard)
        if fullGoodness(blockBoard) < fullGoodness(prevBoard):
            blockBoard = prevBoard
        if blockBoard.count(BLOCKCHAR) >= blockCount: break

    # check if it's valid
    if blockBoard.count(BLOCKCHAR) == blockCount and isConnected(blockBoard):
        printFinal(board, blockBoard)

    # go back if not
    board = initialBoard[:]
    blockBoard = initialBlockBoard[:]
