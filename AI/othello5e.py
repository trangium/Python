...
def negamax(board, mover):
    mm = negamaxHelper(board, mover, getPossibleMoves(board, mover), {i for i in range(64) if board[i] == "."})[:]
...
def quickMove(board, mover, nmDepth=11):
    global cache
    dotCt = board.count(".")
    if dotCt < nmDepth:
...
def main():
    ...
    if possibleMoves:
        prefMove = quickMove(board, mover, 6)
