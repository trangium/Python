...
def negamax(board, mover):
    return alphabeta(board, mover, -65, 65)[:]

def alphabeta(board, mover, lower, upper):
    if not getPossibleMoves(board, mover):
        ...
        ab = alphabeta(board, TOKENINVERSE[mover], -upper, -lower)[:] + [-1]
        ab[0] *= -1
        return ab

    best = [lower-1]
    for pos in getPossibleMoves(board, mover):
        ab = alphabeta(boardOnMove(board, pos, mover), TOKENINVERSE[mover], -upper, -lower)[:]
        score = -ab[0]
