...
def negamaxHelper(board, mover, possibleMoves, dotSet):
...
##    cacheMisses += 1
    ...
    for pos in possibleMoves:
        mm = negamaxHelper(*makeMove(board, pos, mover, dotSet))
...
def negamax(board, mover):
    ...
    mm = negamaxHelper(board, actualMover, getPossibleMoves(board, actualMover), {i for i in range(64) if board[i] == "."})
...
def quickMove(board, mover):
    if board.count(".") < 9: # this can be pushed up to ~7
