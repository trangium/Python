...
import time # Performance: 0.6 to 1.9 seconds on PC
...
OLDALPHA = None
def setGlobals(pzl):
    global ALPHABETSET, ALPHABET, SIZE, NBRLOOKUP, CONSTRAINTSETS, ITERLOOKUP, CONSTRMEMBERS, OLDALPHA
...
def updateTables(mainIndex, mainChar, candidatesTable, hiddenCounts, singles):
...
def bruteHelper(pzl, dotsLeft, prevIndex, candidatesTable, hiddenCounts, singles):
    exhaustiveSet = []
    ...
    while True:
        ...
        minBranch = SIZE
        for testIndex in ITERLOOKUP[singles.pop() if singles else prevIndex]:
            if pzl[testIndex] == ".":
        ...
        for index, character in choices:
            ...
            updateTables(index, character, newCandidatesTable, newHiddenCounts, newSingles)
            bF = bruteHelper(newPzl, dotsLeft-1, index, newCandidatesTable, newHiddenCounts, newSingles)
            if bF: return bF

        # Applying the main choice! Update the candidates table here
        pzl = pzl[:mainIndex] + mainChar + pzl[(mainIndex+1):]
        updateTables(mainIndex, mainChar, candidatesTable, hiddenCounts, singles)
...
def bruteForce(pzl):
    ...
    return bruteHelper(pzl, pzl.count("."), 0, candidatesTable, hiddenCounts, {i for i in range(SIZE**2) if len(candidatesTable[i]) == 1})
