...
# Defaults are: othello5.py, 'x', 1
# Sudent script file must define:
# quickMove(brd, tkn), getPossibleMoves(brd, tkn), and boardOnMove(brd, move, tkn)
# To use a different name, see:   # Import user's file here
# To change the args that getPossibleMoves or boardOnMove is called with, see playGame()
...
def playGame(quickMove, token, show=False):
  ...
  # Uses student's getPossibleMoves(brd, tkn) function and
  #                boardOnMove(brd, tkn, mv)
  ...
  while True:
    ...
    if not (moves:=getPossibleMoves(brd, tknToPlay)):
      tknToPlay = 'xo'[tknToPlay=='x']    # Swap players if pass
      if not (moves:=getPossibleMoves(brd, tknToPlay)): break
    ...
    brd = boardOnMove(brd, mv, tknToPlay)      # , moves   
...
try:
  _tmp = __import__(defFile, globals=None, locals=None,
                    fromlist=['quickMove', 'getPossibleMoves', 'boardOnMove'], level=0)
  quickMove, getPossibleMoves, boardOnMove = _tmp.quickMove, _tmp.getPossibleMoves, _tmp.boardOnMove
except:
...
SECSPERGAME = 1000000                 # normal is .3
