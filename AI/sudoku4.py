...
import time # Performance: 55.06 seconds on grader
...
def bruteHelper(pzl, dotsLeft, prevIndex):
        ...
        for constraintSet in CONSTRAINTSETS:
            for character in ALPHABET:
                ...
                for position in constraintSet:
                    if pzl[position] == character: possible = SIZE*3
                    elif pzl[position] != ".": possible -= 1
                    else: possible -= candidates[character][position]
                if possible < minBranch:
    ...
    if bestCharacter:
        for idxChoice in bestSet:
            if pzl[idxChoice] == "." and isValid(pzl, idxChoice, bestCharacter):
                subPzl = pzl[:idxChoice] + bestCharacter + pzl[(idxChoice+1):]
                bF = bruteHelper(subPzl, dotsLeft-1, idxChoice)
                if bF: return bF
    else:
        for choice in ALPHABET:
            if isValid(pzl, index, choice):
                subPzl = pzl[:index] + choice + pzl[(index+1):]
                bF = bruteHelper(subPzl, dotsLeft-1, index)
                if bF: return bF
    return ""
