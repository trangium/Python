...
##    cacheMisses += 1
    ...
    for pos in possibleMoves:
        ...
        if mover == "x": mm = negamaxHelper(newBoard, newMover, bestSoFar[0], 65, depth+1)
        else: mm = negamaxHelper(newBoard, newMover, -65, bestSoFar[0], depth+1)
        if (mm[0] - bestSoFar[0])*TOKENSCALE[mover] > 0:
            bestSoFar = mm + ([-1, pos] if newMover == mover else [pos])
            if depth == 0: print(bestSoFar)
            if not (lower < mm[0] < upper): break
...
def negamax(board, mover):
    ...
    mm = negamaxHelper(board, actualMover, -65, 65, 0)
...
def quickMove(board, mover):
    if board.count(".") < 8: # this can be pushed up to ~7
