...
HIDDEN = [0 for i in range(82)]
NONHIDDEN = [0 for i in range(82)]
LASTHIDDEN = [0 for i in range(82)]
HIDDENS = []
tSetup = 0
tMain = 0

def setGlobals(pzl):
    global ALPHABETSET, ALPHABET, SIZE, NBRLOOKUP, CONSTRAINTSETS, ITERLOOKUP, CONSTRMEMBERS, HIDDEN, NONHIDDEN, lastHidden
...
def bruteHelper(pzl, dotsLeft, prevIndex, candidatesTable, hiddenCounts):
    exhaustiveSet = []
    ...
    while True:
        ...
        for index, character in choices:
            ...
            updateTables(index, character, newCandidatesTable, newHiddenCounts)
            bF = bruteHelper(newPzl, dotsLeft-1, index, newCandidatesTable, newHiddenCounts)
            if bF: return bF
...
def bruteForce(pzl):
    ...
    return bruteHelper(pzl, pzl.count("."), 0, candidatesTable, hiddenCounts)
...
def timing(startingPzl=0, endingPzl=None):
    ...
    for idx,inp in enumerate(lines[startingPzl:endingPzl]):
        start = time.perf_counter()
        setGlobals(inp)
        tSetup += time.perf_counter()-start
        start = time.perf_counter()
        bruteForce(inp)
        tMain += time.perf_counter()-start
    print(round(time.perf_counter()-absoluteStart, 4),"s total for ",len(lines)," puzzles\n\n", sep="")
