...
def bruteForce(pzl):
    ...
    return bruteHelper(pzl, pzl.count("."), 0, candidatesTable, hiddenCounts, {i for i in range(SIZE**2) if len(candidatesTable[i]) == 1}, {(constrIdx, char) for constrIdx in range(len(CONSTRAINTSETS)) for char in ALPHABET if hiddenCounts[char][constrIdx] == 1})
